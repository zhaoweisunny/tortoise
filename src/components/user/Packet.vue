...
<template>
  <div class="Packet clear">
      ...
      <table border="1" cellspacing="0">
        ...
        <tr v-for="(item, index) in jsonData" :class="{active: item.active}" @click="rowLineHight(item.id)">
          ...
          <th class="op">
            ...
            <a @click="selectUserMessage(item.id)" class="edi">查看手机端用户信息 <img src="../../assets/images/look.png" width="20" alt=""></a>
            <a @click="selectBusMessage(item.id)" class="edi">查看公交端用户信息 <img src="../../assets/images/look.png" width="20" alt=""></a>
          </th>
        </tr>
      </table>
    </div>
    ...
    <packet-edit v-show="showDialog" v-on:update-data="refreshData" :selectId="selectId" :jsonData="jsonData" :opType="opType"></packet-edit>
  </div>
</template>
<script>
...
//  import PhoneUser from './PhoneUser'
  ...
  export default {
    ...
    components: {Operation, Pager, PacketEdit},
    mounted () {
      let that = this
    ...
    methods: {
      goPage (data) {
        this.page = data.page
        this.$router.push({name: 'packet', params: {pageNum: this.page, pageSize: this.pageSize}, query: {ids: this.selectId}})
      },
      getData: function () {   // 请求接口获取数据列表
        let that = this
        this.pageNum = parseInt(this.$route.params.page) || 1
        this.pageSize = parseInt(this.$route.params.pageSize) || 10
        that.$http.post('/alarmcenter/back/TerminalMobileGroup/selectByRetrieval.page', {
      ...
      batchDelete: function () {  // 批量/单行删除数据入口
        ...
        swal({
          ...
          that.$http.post('/alarmcenter/back/TerminalMobileGroup/deleteById', JSON.stringify(that.rowId)
          ).then(
            (response) => {
              if (response.body.code === 200) {
                that.getData()
